package igentuman.immersivenuclear.common.blocks.tileentities;
...
import igentuman.immersivenuclear.common.util.FluidHelper;
...
public class CoolingTowerTileEntity extends PoweredMultiblockTileEntity<CoolingTowerTileEntity, CoolingTowerRecipe> implements IInteractionObjectIE, IBlockBounds{
	/** Input Tank ID */
		...
		if(!isRSDisabled()){
			if(this.energyStorage.getEnergyStored() > 0 && this.processQueue.size() < getProcessQueueMaxLength()){
				if(this.tanks[TANK_INPUT].getFluidAmount() > 0){
					CoolingTowerRecipe recipe = CoolingTowerRecipe.findRecipe(this.tanks[TANK_INPUT].getFluid());
					if(recipe != null && this.tanks[TANK_INPUT].getFluidAmount() >= recipe.getInputFluid().getAmount() && this.energyStorage.getEnergyStored() >= recipe.getTotalProcessEnergy()){
						MultiblockProcessInMachine<CoolingTowerRecipe> process = new MultiblockProcessInMachine<CoolingTowerRecipe>(recipe).setInputTanks(TANK_INPUT);
	...
	@Override
	protected CoolingTowerRecipe getRecipeForId(ResourceLocation id){
		return CoolingTowerRecipe.recipes.get(id);
	}
	...
	protected IFluidTank[] getAccessibleFluidTanks(Direction side){
		CoolingTowerTileEntity master = master();
		if(master != null){
			// Fluid Input
		...
		if(this.posInMultiblock.equals(Fluid_IN)){
			if(getIsMirrored() ? (side == null || side == getFacing().rotateYCCW()) : (side == null || side == getFacing().rotateY())){
				...
				if(master.tanks[TANK_INPUT].getFluid() == FluidStack.EMPTY){
					CoolingTowerRecipe r = CoolingTowerRecipe.findRecipe(copy0);
					return r != null;
				}else{
					CoolingTowerRecipe r0 = CoolingTowerRecipe.findRecipe(copy0);
					CoolingTowerRecipe r1 = CoolingTowerRecipe.findRecipe(copy1);
					return r0 == r1;
				}
			}
		}
	...
	private static CachedShapesWithTransform<BlockPos, Pair<Direction, Boolean>> SHAPES = CachedShapesWithTransform.createForMultiblock(CoolingTowerTileEntity::getShape);
