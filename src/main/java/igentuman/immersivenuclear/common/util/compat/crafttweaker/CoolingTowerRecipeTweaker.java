...
@ZenRegister
@Name("mods.immersivenuclear.DistillationTower")
public class CoolingTowerRecipeTweaker {
	
	@Method
	public static boolean remove(String name){
		List<ResourceLocation> test = CoolingTowerRecipe.recipes.keySet().stream()
		...
		if(test.size() > 1){
			...
			if(CoolingTowerRecipe.recipes.containsKey(id)){
				CoolingTowerRecipe.recipes.remove(id);
				return true;
			}else{
	...
	public static void removeAll(){
		CoolingTowerRecipe.recipes.clear();
	}
	
	@ZenRegister
	@Name("mods.immersivenuclear.DistillationBuilder")
	public static class DistillationRecipeBuilder{
		...
		public void build(String name){
			...
			if(this.isValid){
				...
				CoolingTowerRecipe recipe = new CoolingTowerRecipe(id, fluidOutStacks, fluidInTag, this.fluxEnergy, this.timeTicks, chances);
				CoolingTowerRecipe.recipes.put(id, recipe);
			}
		}
	}
}
