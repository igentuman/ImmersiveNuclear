...
public class ReservoirCommand{
	...
	// new TranslationTextComponent("chat.immersivenuclear.command.reservoir.setCapacity.NFE", numberString)
	...
	static int get(ServerPlayerEntity playerEntity){
		...
		TranslationTextComponent cmp = new TranslationTextComponent("chat.immersivenuclear.command.reservoir.get",
	...
	static void setReservoir(CommandContext<CommandSource> context, int xChunk, int zChunk){
		...
		if(reservoir == null){
			sender.sendFeedback(new TranslationTextComponent("chat.immersivenuclear.command.reservoir.set.invalidReservoir", name), true);
			return;
		}
		
		info.overrideType = reservoir;
		IPSaveData.markInstanceAsDirty();
		sender.sendFeedback(new TranslationTextComponent("chat.immersivenuclear.command.reservoir.set.sucess", reservoir.name), true);
	}
	...
	static int set(ServerPlayerEntity playerEntity, String name){
		...
		if(reservoir == null){
			playerEntity.sendMessage(new TranslationTextComponent("chat.immersivenuclear.command.reservoir.set.invalidReservoir", name), Util.DUMMY_UUID);
			return Command.SINGLE_SUCCESS;
		}
		
		info.overrideType = reservoir;
		playerEntity.sendMessage(new TranslationTextComponent("chat.immersivenuclear.command.reservoir.set.sucess", reservoir.name), Util.DUMMY_UUID);
		IPSaveData.markInstanceAsDirty();
	...
	static int setAmount(ServerPlayerEntity playerEntity, int amount){
		...
		info.current = amount;
		playerEntity.sendMessage(new TranslationTextComponent("chat.immersivenuclear.command.reservoir.setAmount.sucess", Integer.toString(amount)), Util.DUMMY_UUID);
		IPSaveData.markInstanceAsDirty();
	...
	static int setCapacity(ServerPlayerEntity playerEntity, int amount){
		...
		playerEntity.sendMessage(new TranslationTextComponent("chat.immersivenuclear.command.reservoir.setCapacity.sucess", Integer.toString(amount)), Util.DUMMY_UUID);
		IPSaveData.markInstanceAsDirty();
