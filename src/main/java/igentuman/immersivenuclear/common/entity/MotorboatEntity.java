package igentuman.immersivenuclear.common.entity;
...
import igentuman.immersivenuclear.common.network.MessageConsumeBoatFuel;
import igentuman.immersivenuclear.common.util.IPItemStackHandler;
...
public class MotorboatEntity extends BoatEntity implements IEntityAdditionalSpawnData{
	...
	private static EntityType<MotorboatEntity> createType(){
		EntityType<MotorboatEntity> ret = EntityType.Builder.<MotorboatEntity> create(MotorboatEntity::new, EntityClassification.MISC).size(1.375F, 0.5625F).build(ImmersiveNuclear.MODID + ":speedboat");
		ret.setRegistryName(ImmersiveNuclear.MODID, "speedboat");
		return ret;
	}
		...
		if(key == UPGRADE_0 || key == UPGRADE_1 || key == UPGRADE_2 || key == UPGRADE_3){
			...
			for(ItemStack upgrade:upgrades){
				if(upgrade != null && upgrade != ItemStack.EMPTY){
					...
					}else if(item == IPContent.BoatUpgrades.ice_breaker){
						this.hasIcebreaker = true;
					}else if(item == IPContent.BoatUpgrades.tank){
						this.hasTank = true;
					}else if(item == IPContent.BoatUpgrades.rudders){
						this.hasRudders = true;
					}else if(item == IPContent.BoatUpgrades.paddles){
		...
		if(this.world.isRemote){
			if(!isEmergency()){
				...
				if(lastMoving != moving){
					ImmersiveNuclear.proxy.handleEntitySound(IESounds.dieselGenerator, this, false, .5f, 0.5F);
				}
				ImmersiveNuclear.proxy.handleEntitySound(IESounds.dieselGenerator, this, this.isBeingRidden() && this.getContainedFluid() != FluidStack.EMPTY && this.getContainedFluid().getAmount() > 0, this.forwardInputDown || this.backInputDown ? .5f : .3f, moving);
				lastMoving = moving;
