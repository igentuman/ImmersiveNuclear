...
public class IPFluid extends FlowingFluid{
	...
	public IPFluid(String name, int density, int viscosity){
		this(name,
				new ResourceLocation(ImmersiveNuclear.MODID, "block/fluid/" + name + "_still"),
				new ResourceLocation(ImmersiveNuclear.MODID, "block/fluid/" + name + "_flow"), IPFluid.createBuilder(density, viscosity));
	}
	...
	protected IPFluid(String name, ResourceLocation stillTexture, ResourceLocation flowingTexture, @Nullable Consumer<FluidAttributes.Builder> buildAttributes, boolean isSource){
		...
		if(!isSource){
			flowing = this;
			setRegistryName(ImmersiveNuclear.MODID, this.fluidName + "_flowing");
		}else{
			this.source = this;
			...
			setRegistryName(new ResourceLocation(ImmersiveNuclear.MODID, this.fluidName));
	...
	public static class IPFluidBlock extends FlowingFluidBlock{
		...
		public IPFluidBlock(IPFluid fluid, String fluidName){
			...
			this.fluid = fluid;
			setRegistryName(new ResourceLocation(ImmersiveNuclear.MODID, fluidName + "_fluid_block"));
		}
	...
	public static class IPBucketItem extends BucketItem{
		private static final Item.Properties PROPS = new Item.Properties().maxStackSize(1).group(ImmersiveNuclear.creativeTab);
		
		public IPBucketItem(IPFluid fluid, String fluidName){
			super(() -> fluid, PROPS);
			setRegistryName(new ResourceLocation(ImmersiveNuclear.MODID, fluidName + "_bucket"));
		}
