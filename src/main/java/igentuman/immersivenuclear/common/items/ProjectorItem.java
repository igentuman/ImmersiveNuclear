...
import igentuman.immersivenuclear.common.util.projector.MultiblockProjection;
import igentuman.immersivenuclear.common.util.projector.Settings;
...
import igentuman.immersivenuclear.api.event.ProjectorEvent;
import igentuman.immersivenuclear.client.ClientProxy;
import igentuman.immersivenuclear.client.ShaderUtil;
import igentuman.immersivenuclear.client.render.IPRenderTypes;
import igentuman.immersivenuclear.common.IPContent;
import igentuman.immersivenuclear.common.IPContent.Items;
...
public class ProjectorItem extends IPItemBase{
	...
	public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn){
		...
		if(settings.getMultiblock() != null){
			...
			tooltip.add(new TranslationTextComponent("desc.immersivenuclear.info.projector.build0"));
			tooltip.add(new TranslationTextComponent("desc.immersivenuclear.info.projector.build1", new TranslationTextComponent("desc.immersiveengineering.info.multiblock.IE:" + name)));
			
			if(isPressing(GLFW.GLFW_KEY_LEFT_SHIFT) || isPressing(GLFW.GLFW_KEY_RIGHT_SHIFT)){
				ITextComponent title = new TranslationTextComponent("desc.immersivenuclear.info.projector.holdshift.text").mergeStyle(TextFormatting.DARK_AQUA);
				tooltip.add(title);
				
				ITextComponent mbSize = new TranslationTextComponent("desc.immersivenuclear.info.projector.size", size.getX(), size.getY(), size.getZ()).mergeStyle(TextFormatting.DARK_GRAY);
				tooltip.add(indent(mbSize));
				
				Direction dir = Direction.byHorizontalIndex(settings.getRotation().ordinal());
				ITextComponent rotation = new TranslationTextComponent("desc.immersivenuclear.info.projector.rotated." + dir).mergeStyle(TextFormatting.DARK_GRAY);
				
				ITextComponent flip;
				if(settings.isMirrored()){
					flip = new TranslationTextComponent("desc.immersivenuclear.info.projector.flipped.true").mergeStyle(TextFormatting.DARK_GRAY);
				}else{
					flip = new TranslationTextComponent("desc.immersivenuclear.info.projector.flipped.false").mergeStyle(TextFormatting.DARK_GRAY);
				}
				...
				if(settings.getPos() != null){
					...
					ITextComponent centerText = new TranslationTextComponent("desc.immersivenuclear.info.projector.center", x, y, z).mergeStyle(TextFormatting.DARK_GRAY);
				...
				ITextComponent text = new StringTextComponent("[")
						.appendSibling(new TranslationTextComponent("desc.immersivenuclear.info.projector.holdshift"))
						.appendString("] ")
						.appendSibling(new TranslationTextComponent("desc.immersivenuclear.info.projector.holdshift.text"))
						.mergeStyle(TextFormatting.DARK_AQUA);
				tooltip.add(text);
			}
			
			if(isPressing(GLFW.GLFW_KEY_LEFT_CONTROL) || isPressing(GLFW.GLFW_KEY_RIGHT_CONTROL)){
				ITextComponent title = new TranslationTextComponent("desc.immersivenuclear.info.projector.holdctrl.text").mergeStyle(TextFormatting.DARK_PURPLE);
				ITextComponent ctrl0 = new TranslationTextComponent("desc.immersivenuclear.info.projector.control1").mergeStyle(TextFormatting.DARK_GRAY);
				ITextComponent ctrl1 = new TranslationTextComponent("desc.immersivenuclear.info.projector.control2", ClientProxy.keybind_preview_flip.func_238171_j_()).mergeStyle(TextFormatting.DARK_GRAY);
				ITextComponent ctrl2 = new TranslationTextComponent("desc.immersivenuclear.info.projector.control3").mergeStyle(TextFormatting.DARK_GRAY);
				...
				ITextComponent text = new StringTextComponent("[")
						.appendSibling(new TranslationTextComponent("desc.immersivenuclear.info.projector.holdctrl"))
						.appendString("] ")
						.appendSibling(new TranslationTextComponent("desc.immersivenuclear.info.projector.holdctrl.text"))
						.mergeStyle(TextFormatting.DARK_PURPLE);
				tooltip.add(text);
			}
		}else{
			tooltip.add(new TranslationTextComponent("desc.immersivenuclear.info.projector.noMultiblock"));
		}
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn){
		...
		switch(settings.getMode()){
			...
			case MULTIBLOCK_SELECTION:{
				if(worldIn.isRemote){
					if(!playerIn.isSneaking()){
						ImmersiveNuclear.proxy.openProjectorGui(handIn, held);
					}else{
						changeMode = true;
					}
				}
	...
	public static class ClientInputHandler{
			...
			if(shiftHeld && delta != 0.0){
				...
				if(main || off){
					...
					if(shiftHeld){
						...
						player.sendStatusMessage(new TranslationTextComponent("desc.immersivenuclear.info.projector.rotated." + facing), true);
		...
		private static void doAFlip(){
			...
			if(main || off){
				...
				ITextComponent flip;
				if(settings.isMirrored()){
					flip = new TranslationTextComponent("desc.immersivenuclear.info.projector.flipped.true");
				}else{
					flip = new TranslationTextComponent("desc.immersivenuclear.info.projector.flipped.false");
				}
