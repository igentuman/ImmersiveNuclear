...
public class MotorboatItem extends IPItemBase implements IUpgradeableTool{
	...
	public MotorboatItem(String name){
		super(name, new Item.Properties().maxStackSize(1).group(ImmersiveNuclear.creativeTab));
	}
	...
	protected NonNullList<ItemStack> getContainedItems(ItemStack stack){
		...
		if(handler == null){
			ImmersiveNuclear.log.info("No valid inventory handler found for " + stack);
		...
		ImmersiveNuclear.log.warn("Inefficiently getting contained items. Why does " + stack + " have a non-IE IItemHandler?");
