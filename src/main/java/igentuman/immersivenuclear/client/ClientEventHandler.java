...
import igentuman.immersivenuclear.api.crafting.LubricatedHandler.ILubricationHandler;
		...
		if(stack.getItem() instanceof CoresampleItem){
			if(ItemNBTHelper.hasKey(stack, "resAmount")){
				...
				if(res != null && amnt > 0){
					...
					ITextComponent header = new TranslationTextComponent("chat.immersivenuclear.info.coresample.oil", fluidName).mergeStyle(TextFormatting.GRAY);
					...
					if(res != null && res.replenishRate > 0){
						...
						ITextComponent info = new StringTextComponent("  " + I18n.format("chat.immersivenuclear.info.coresample.oilRep", res.replenishRate, fluidName)).mergeStyle(TextFormatting.GRAY);
						
						tooltip.add(tipPos, header);
						tooltip.add(tipPos + 1, info);
					}else{
						tooltip.add(tipPos, new StringTextComponent(I18n.format("chat.immersivenuclear.info.coresample.noOil")).mergeStyle(TextFormatting.GRAY));
					}
				}
			}
		}
							...
							if(mop instanceof BlockRayTraceResult){
								...
								if(tileEntity instanceof CoresampleTileEntity){
									...
									if(ItemNBTHelper.hasKey(coresample, "resAmount") && text != null && text.length > 0){
										...
										ITextComponent display = new TranslationTextComponent("chat.immersivenuclear.info.coresample.noOil");
										
										if(res != null && amnt > 0){
											ITextComponent fluidName = new FluidStack(res.getFluid(), 1).getDisplayName();
											display = new TranslationTextComponent("chat.immersivenuclear.info.coresample.oil", fluidName);
										}else if(res != null && res.replenishRate > 0){
											ITextComponent fluidName = new FluidStack(res.getFluid(), 1).getDisplayName();
											display = new TranslationTextComponent("chat.immersivenuclear.info.coresample.oilRep", res.replenishRate, fluidName);
										}
