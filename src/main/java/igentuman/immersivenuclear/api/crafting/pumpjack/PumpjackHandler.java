...
public class PumpjackHandler{
	...
	public static ReservoirWorldInfo getOrCreateOilWorldInfo(World world, DimensionChunkCoords coords, boolean force){
		...
		if(worldInfo == null){
			...
			ImmersiveNuclear.log.debug("Empty? {}. Forced? {}. Size: {}, Query: {}", empty ? "Yes" : "No", force ? "Yes" : "No", size, query);
			...
			if(!empty || force){
				...
				ImmersiveNuclear.log.debug(coords.dimension.getLocation());
				
				int totalWeight = getTotalWeight(dimension, biome);
				ImmersiveNuclear.log.debug("Total Weight: " + totalWeight);
				if(totalWeight > 0){
			...
			if(res != null){
				ImmersiveNuclear.log.debug("Using: {}", res.name);
				
				capacity = (int) ((res.maxSize - res.minSize) * size + res.minSize);
			}
			
			ImmersiveNuclear.log.debug("Capacity: {}", capacity);
			...
			ImmersiveNuclear.log.debug("Storing {} for {}", worldInfo, coords);
	...
	public static class ReservoirType extends IESerializableRecipe{
		public static final IRecipeType<ReservoirType> TYPE = IRecipeType.register(ImmersiveNuclear.MODID + ":reservoirtype");
