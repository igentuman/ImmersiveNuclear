package igentuman.immersivenuclear.api.crafting.builders;
...
public class CoolingTowerRecipeBuilder extends IEFinishedRecipe<CoolingTowerRecipeBuilder>{
	...
	public static CoolingTowerRecipeBuilder builder(FluidStack... fluidOutput){
		...
		CoolingTowerRecipeBuilder b = new CoolingTowerRecipeBuilder();
	 ...
	 */
	public CoolingTowerRecipeBuilder addByproduct(ItemStack byproduct, int chance){
	 ...
	 * @return self for chaining
	 */
	public CoolingTowerRecipeBuilder addByproduct(ItemStack byproduct, double chance){
		this.byproducts.add(new Tuple<ItemStack, Double>(byproduct, Math.max(Math.min(chance, 1.0), 0.0)));
		return this;
	}
	
	public CoolingTowerRecipeBuilder setTimeAndEnergy(int time, int energy){
		return setTime(time).setEnergy(energy);
	}
	
	public CoolingTowerRecipeBuilder addInput(ITag.INamedTag<Fluid> fluidTag, int amount){
		return addFluidTag("input", fluidTag, amount);
	}
	
	public CoolingTowerRecipeBuilder addInput(Fluid fluid, int amount){
		return addInput(new FluidStack(fluid, amount));
	}
	
	public CoolingTowerRecipeBuilder addInput(FluidStack fluidStack){
		return addFluid("input", fluidStack);
	}
	
	public CoolingTowerRecipeBuilder addFluids(String key, FluidStack... fluidStacks){
	...
	public CoolingTowerRecipeBuilder addItems(String key, ItemStack... itemStacks){
		return addWriter(jsonObject -> {
			JsonArray array = new JsonArray();
	...
	private static final CoolingTowerRecipeBuilder dummy = new CoolingTowerRecipeBuilder();
