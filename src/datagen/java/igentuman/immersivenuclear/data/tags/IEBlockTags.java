...
import igentuman.immersivenuclear.common.blocks.generic.ConnectorBlock;
import igentuman.immersivenuclear.common.blocks.generic.ScaffoldingBlock;
import igentuman.immersivenuclear.common.blocks.metal.ConveyorBlock;
import igentuman.immersivenuclear.common.blocks.metal.MetalLadderBlock;
...
import igentuman.immersivenuclear.common.register.INBlocks.*;
import igentuman.immersivenuclear.common.util.IELogger;
...
public class IEBlockTags extends BlockTagsProvider
{
	...
	protected void addTags(Provider p_256380_)
	{
		...
		for(EnumMetals metal : EnumMetals.values())
		{
			...
			tag(tags.sheetmetal).add(INBlocks.Metals.SHEETMETAL.get(metal).get());
			tag(IETags.sheetmetals).addTag(tags.sheetmetal);
			tag(IETags.sheetmetalSlabs).add(INBlocks.TO_SLAB.get(Metals.SHEETMETAL.get(metal).getId()).get());
		}
		for(DyeColor dye : DyeColor.values())
		{
			tag(IETags.sheetmetals).add(MetalDecoration.COLORED_SHEETMETAL.get(dye).get());
			tag(IETags.sheetmetalSlabs).add(INBlocks.TO_SLAB.get(MetalDecoration.COLORED_SHEETMETAL.get(dye).getId()).get());
		}
		for(TreatedWoodStyles style : TreatedWoodStyles.values())
		{
			tag(IETags.treatedWood).add(WoodenDecoration.TREATED_WOOD.get(style).get());
			tag(IETags.treatedWoodSlab).add(INBlocks.TO_SLAB.get(INBlocks.WoodenDecoration.TREATED_WOOD.get(style).getId()).get());
		}
		...
		for(final BlockEntry<ScaffoldingBlock> entry : INBlocks.MetalDecoration.STEEL_SCAFFOLDING.values())
		{
			tag(IETags.scaffoldingSteelStair).add(INBlocks.TO_STAIRS.get(entry.getId()).get());
			tag(IETags.scaffoldingSteelSlab).add(INBlocks.TO_SLAB.get(entry.getId()).get());
		}
		for(final BlockEntry<ScaffoldingBlock> entry : INBlocks.MetalDecoration.ALU_SCAFFOLDING.values())
		{
			tag(IETags.scaffoldingAluStair).add(INBlocks.TO_STAIRS.get(entry.getId()).get());
			tag(IETags.scaffoldingAluSlab).add(INBlocks.TO_SLAB.get(entry.getId()).get());
		}
		...
		{
			tag(BlockTags.WOODEN_SLABS).add(INBlocks.TO_SLAB.get(treatedWood.getId()).get());
			tag(BlockTags.WOODEN_STAIRS).add(INBlocks.TO_STAIRS.get(treatedWood.getId()).get());
		}
		for(BlockEntry<?> slab : INBlocks.TO_SLAB.values())
			tag(BlockTags.SLABS).add(slab.get());
		for(BlockEntry<?> stairs : INBlocks.TO_STAIRS.values())
			tag(BlockTags.STAIRS).add(stairs.get());
		for(BlockEntry<?> stairs : INBlocks.TO_WALL.values())
	...
	private void registerHammerMineable()
	{
		...
		for(RegistryObject<Block> regObject : INBlocks.REGISTER.getEntries())
		{
	...
	private void registerGrindingDiskMineable()
	{
		...
		for(EnumMetals metal : EnumMetals.values())
			if(!metal.isVanillaMetal())
				tag.add(INBlocks.TO_SLAB.get(Metals.STORAGE.get(metal).getId()).get());
	...
	private void registerMineable(IntrinsicTagAppender<Block> tag, List<BlockEntry<?>> entries)
	{
		...
		{
			tag.add(entry.get());
			BlockEntry<?> slab = INBlocks.TO_SLAB.get(entry.getId());
			if(slab!=null)
				tag.add(slab.get());
			BlockEntry<?> stairs = INBlocks.TO_STAIRS.get(entry.getId());
			if(stairs!=null)
				tag.add(stairs.get());
			BlockEntry<?> wall = INBlocks.TO_WALL.get(entry.getId());
			if(wall!=null)
				tag.add(wall.get());
		}
	}
	...
	private void checkAllRegisteredForBreaking()
	{
		...
		Set<ResourceLocation> knownNonHarvestable = Stream.of(
				...
				.collect(Collectors.toSet());
		Set<ResourceLocation> registered = INBlocks.REGISTER.getEntries().stream()
