...
import igentuman.immersivenuclear.common.blocks.IEStairsBlock;
import igentuman.immersivenuclear.common.blocks.IEWallBlock;
...
public abstract class ExtendedBlockstateProvider extends BlockStateProvider
{
	...
	{
		SlabBlock b = INBlocks.TO_SLAB.get(BuiltInRegistries.BLOCK.getKey(full.get())).get();
	...
	{
		final IEStairsBlock b = INBlocks.TO_STAIRS.get(BuiltInRegistries.BLOCK.getKey(full.get())).get();
	...
	{
		final IEWallBlock b = INBlocks.TO_WALL.get(BuiltInRegistries.BLOCK.getKey(full.get())).get();
	...
	{
		return models().withExistingParent(name, ImmersiveNuclear.rl("block/"+type))
	...
	{
		return models().withExistingParent(name, ImmersiveNuclear.rl("block/wall_inventory_topped"))
				.texture("wall_bottom", bottom)
				.texture("wall_side", side)
				.texture("wall_top", top);
	}
